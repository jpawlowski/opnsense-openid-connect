...
| [OAuth 2.0 Bearer Token Usage (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.html) | Bearer access tokens used when DPoP is not negotiated | Partial | 🟡 unverified | Only client-side use needed by this RP is in scope. |
...
| [OAuth 2.0 Demonstrating Proof of Possession (RFC 9449)](https://www.rfc-editor.org/rfc/rfc9449.html) | Sender-constrained access tokens for the confidential web RP | Implemented | 🟡 unverified | ES256 DPoP is negotiated from Discovery, authorization codes and protected requests are proof-key-bound, server nonces are retained without downgrade, and per-provider keys rotate with bounded retired generations. The complete normative inventory is not yet audited. |
