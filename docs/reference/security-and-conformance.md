...
| [OAuth 2.0 Bearer Token Usage (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.html) | Bearer access token sent by the RP to UserInfo when DPoP is not negotiated | Implemented | ✅ verified | Verified for client-side Bearer header use at the protected UserInfo resource; token issuance, resource-server responses and revocation form semantics are outside this claim. |
...
| [OAuth 2.0 Mutual-TLS Client Authentication and Certificate-Bound Tokens (RFC 8705)](https://www.rfc-editor.org/rfc/rfc8705.html) | Token endpoint client authentication and sender-constrained tokens | Implemented | 🟡 unverified | PKI and self-signed mutual-TLS client authentication, endpoint aliases, OPNsense certificate selection and rotation, and certificate-bound UserInfo use are implemented; requirement-level interoperability evidence is incomplete. |
| [JWT Profile for OAuth 2.0 Client Authentication (RFC 7523)](https://www.rfc-editor.org/rfc/rfc7523.html) | private_key_jwt client authentication | Implemented | 🟡 unverified | Implemented with OPNsense certificate custody and endpoint-specific metadata negotiation; the complete normative inventory remains unfinished. |
...
| [OAuth 2.0 Demonstrating Proof of Possession (RFC 9449)](https://www.rfc-editor.org/rfc/rfc9449.html) | Sender-constrained access tokens for the confidential web RP | Implemented | 🟡 unverified | ES256 DPoP is negotiated for compatible documented access-token profiles from Discovery, authorization codes and protected requests are proof-key-bound, server nonces are retained without downgrade, and per-provider keys rotate with bounded retired generations. The complete normative inventory is not yet audited. |
...
| [JWK Thumbprint URI (RFC 9278)](https://www.rfc-editor.org/rfc/rfc9278.html) | URI identification of proof-of-possession keys | N/A | N/A | DPoP uses the RFC 7638 thumbprint value directly, not the URI form defined by this specification. |
| [Proof-of-Possession Key Semantics for JWTs (RFC 7800)](https://www.rfc-editor.org/rfc/rfc7800.html) | JWT confirmation claims for proof-of-possession keys | N/A | N/A | The provider and protected resource process the DPoP access token confirmation claim; this RP acts only as the presenting client. |
| [Assertion Framework for OAuth 2.0 (RFC 7521)](https://www.rfc-editor.org/rfc/rfc7521.html) | Assertion grants and client authentication | Partial | 🟡 unverified | The RFC 7523 private_key_jwt client-authentication use is implemented; assertion grants remain outside WebGUI login. |
...
| [Auth0](../providers/auth0.md) | [📘](https://auth0.com/docs/get-started/authentication-and-authorization-flow/authorization-code-flow) | ? | ? | ? | ? | ? | ? | ? | ? | ? | [📘](https://auth0.com/docs/secure/multi-factor-authentication/step-up-authentication/configure-step-up-authentication-for-web-apps) | ? |
...
| Named provider presets cover every provider-dependent field, and generated Keycloak or authentik setup files bind visible naming, branding and exact redirects to the active accepted WebGUI FQDN without embedding a client secret. | [src/opnsense/mvc/app/library/OPNsense/Auth/OpenIDConnect.php](../../src/opnsense/mvc/app/library/OPNsense/Auth/OpenIDConnect.php)<br/>[src/opnsense/mvc/app/library/OPNsense/OpenIDConnect/ProviderSetup.php](../../src/opnsense/mvc/app/library/OPNsense/OpenIDConnect/ProviderSetup.php) | current host-independent regression suite: [tests/unit/settings.php](../../tests/unit/settings.php), [tests/unit/provider-setup.php](../../tests/unit/provider-setup.php) |
