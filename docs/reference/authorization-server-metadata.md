...
| `token_endpoint_auth_methods_supported` | optional non-empty string list; omission means `client_secret_basic`; automatic or configured selection is limited to advertised `client_secret_basic` or `client_secret_post` |
| `token_endpoint_auth_signing_alg_values_supported` | ignored because JWT client authentication is unsupported |
...
| `revocation_endpoint_auth_signing_alg_values_supported` | ignored because JWT client authentication is unsupported |
| `introspection_endpoint` and its authentication metadata | ignored; this RP does not introspect tokens |
