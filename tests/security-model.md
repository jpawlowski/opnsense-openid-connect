...
| account/configuration enumeration | missing, disabled, expired, privileged, approval-pending and otherwise unusable accounts receive the same status, styled public refusal and same-shaped random reference; only an administrator can correlate a real approval reference or the audit reason |
