...
| open redirect/Host injection | local target sanitizer, strict Host grammar, HTTPS origin matched to OPNsense hostname/domain, alternate hostnames and core's IP-literal rule at the configured port, an explicitly enabled standard-port rule limited to those DNS names, or an explicit exact custom list; disabling OPNsense DNS-rebinding checks does not widen hostname acceptance |
...
| any global social account reaches the firewall | an unknown identity is never given a session; Administrator approval queues an exact issuer/subject in bounded 0600 storage, while the browser receives the same styled refusal and same-shaped reference as every unusable local-account outcome, with no unauthenticated config write |
| unauthorized identity rebinding, account creation or local-group assignment | the manager API extends core's System: Authentication Servers ACL, repeats that privilege check in the controller and applies `user-config-readonly` to every mutation; inline accounts and membership changes additionally require core's System: Access: Management privilege; the complete selection is limited to existing groups and saved atomically with the binding, while inline accounts retain a scrambled password; binding operations target one exact saved server and use record IDs to detect concurrent changes |
...
| account/configuration enumeration | missing, disabled, expired, privileged, approval-pending and otherwise unusable accounts receive the same status, styled public refusal and a fresh same-shaped random reference on every attempt; only the audit log maps that public reference to an internal approval request or refusal reason |
