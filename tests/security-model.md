...
| any global social account reaches the firewall | an unknown identity is never given a session; Administrator approval queues an exact issuer/subject in bounded 0600 storage and returns only a short workflow reference after successful provider authentication, with no unauthenticated config write |
...
| account/configuration enumeration | missing, disabled, expired, privileged and otherwise unusable accounts receive the same public refusal; a newly queued approval is distinguished only after successful provider authentication and reveals a random workflow reference but no local-account details; precise reasons remain in the log |
